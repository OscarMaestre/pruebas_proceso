...
for linea in lineas:
    ...
    if (codigo_centro!=utilidades.NO_CONCORDANCIA):
        ...
        nombre_centro=linea[34:pos_comienzo_localidad].strip()
        nombre_centro=utilidades.quitar_poblacion_parentesis(nombre_centro).strip()
        sql_insert_cepa+=generar_insert_cp(codigo_centro, nombre_centro, codigo_localidad, "CPM")
        
utilidades.escribir_texto_en_fichero("insert_cpms.sql", sql_insert_cepa);